...
function sendRejectionEmail($email) {
    ...
    try {
        ...
        $mail->Password   = 'tndkjgrmzvrSftcl';                  // SMTP password
...
function sendRejectionSMS($phone) {
    // Twilio credentials
    $sid = 'AC1b643fe8e89419ad94b3332c8ad396c3'; // Your Twilio Account SID
    $token = '0321c66c56cb7546795d2451947d739e'; // Your Twilio Auth Token
    $twilio_phone_number = '+18647744573'; // Your valid Twilio phone number
    ...
    if ($_POST['action'] === 'approve') {
        ...
        if ($donor_email) {
            ...
            if ($email_count > 0) {
                ...
                $db->prepare("UPDATE pending_donors SET status = 'Rejected' WHERE donor_id = :donor_id")->execute([':donor_id' => $donor_id]);
                ...
                // Update status in the database
                $db->prepare("UPDATE pending_donors SET status = 'Accepted' WHERE donor_id = :donor_id")->execute([':donor_id' => $donor_id]);
